...
### Fixed
- **Messages typed while a self-compact was pending ran on the stale context** — `request_compact` defers the actual compact to `agent_settled`, but a message typed in that window (Enter → steering queue, Alt+Enter → follow-up) is processed by pi's post-run loop *before* the agent settles — the user's turn ran on the un-compacted context and the compact was delayed until that run ended (field: the compact only fired when the run was aborted). The input handler now swallows interactive messages while `pendingCompact` is set (`{ action: "handled" }`, with a "message saved" notify); `session_compact` re-delivers them after the compaction (200 ms — `session_compact` fires while pi's compaction-in-progress flag is still set and `prompt()` refuses to submit during compaction), in order, via `sendUserMessage` (followUp when a run is active, direct when idle). The stashed messages *are* the continuation, so the generic proceed steer is skipped — including the manual path's `onComplete` (suppressed via `stashedRedeliveryPending`). `session_compact_failed` delivers the stash immediately so failed compacts never drop user input; `resetAllState` discards the stash and any pending redelivery timer (a new session keeps none of it). Extension-source input (gallop's own steers/nudges) is never swallowed; once a compact is actually running, the TUI routes typed messages through its own compaction queue and they never reach the gate.
- **Stale pressure nudge right after a model-initiated compact** — pi emits `message_end` *before* a message's tool calls execute, so when the model's final message *was* the `request_compact` call, the nudge's en-route guards (`pendingCompact`, `compactionRunning`) saw nothing — the tool's `execute` had not run yet. The nudge steer was queued around the compaction boundary and reached the context as "call request_compact now" immediately after the model had just called the tool (rendered between the user's compact command and the completion marker — a live instruction that read as stale). `checkContextNudge` now also skips messages that contain a `request_compact` tool call: the compact is already en route and the call terminates the run, so a nudge can only be noise. The suite now exercises the real pi order (message_end before execute) — the pre-existing pending-compact test ran the tool first, the reverse order, which is how this slipped through.
